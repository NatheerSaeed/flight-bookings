...
{
    public static Packages ToDto(this Package model)
    {
        return new Packages
        {
            ...
            GalleriesItems = model.GalleriesItems?.Select(x => x.Id).ToList(),
            GoodToKnowsItems = model.GoodToKnowsItems?.Select(x => x.Id).ToList(),
            HotelDealsItems = model.HotelDealsItems?.Select(x => x.Id).ToList(),
            Id = model.Id,
            PackageAttractionsItems = model.PackageAttractionsItems?.Select(x => x.Id).ToList(),
    ...
    public static Package ToModel(
        this PackageUpdateInput updateDto,
        PackagesWhereUniqueInput uniqueId
    )
    {
        var packages = new Package { Id = uniqueId.Id };
